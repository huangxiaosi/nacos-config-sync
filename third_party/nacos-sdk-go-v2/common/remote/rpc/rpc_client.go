...
// rpcKeepAliveInterval is the idle period before sending an application-level HealthCheckRequest.
// Set process env NACOS_SDK_RPC_KEEP_ALIVE_SECONDS to a positive integer (seconds) to override
// the default constant.KEEP_ALIVE_TIME (5).
func rpcKeepAliveInterval() time.Duration {
	defaultSec := int(constant.KEEP_ALIVE_TIME)
	if v := strings.TrimSpace(os.Getenv("NACOS_SDK_RPC_KEEP_ALIVE_SECONDS")); v != "" {
...
func (r *RpcClient) Start() {
	...
	go func() {
		timer := time.NewTimer(rpcKeepAliveInterval())
		for {
			select {
...
func (r *RpcClient) healthCheck(timer *time.Timer) {
	defer timer.Reset(rpcKeepAliveInterval())
	var reconnectContext ReconnectContext
	lastActiveTimeStamp := r.lastActiveTimestamp.Load().(time.Time)
	if time.Now().Sub(lastActiveTimeStamp) < rpcKeepAliveInterval() {
		return
	}
